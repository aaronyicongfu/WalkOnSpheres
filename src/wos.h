...
class WOS {
 public:
  ...
  arrayd<N> evalGrad(arrayd<N> &x, double t) const {
    ...
    arrayd<N> randOnSphere = randPointOnSphere(x, R);
    arrayd<N> sol =
        times(subtract(randOnSphere, x), N * eval(randOnSphere, t) / (R * R));
    if (pde->type == PDETypes::Poisson) {
      arrayd<N> randInSphere = randPointInSphere(x, R);
      arrayd<N> source =
          times(gradG(x, randInSphere, R),
                M_PI * R * R * pde->laplacianOP(randInSphere, t));
      return add(sol, source);
    }
    return sol;
  }
  double truth(arrayd<N> &x, double t) const { return pde->truth(x, t); }
